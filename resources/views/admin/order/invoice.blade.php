                                ...
                                <table id="example2" class="table table-hover dt-responsive" cellspacing="0"
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td width="50%">
                                                ...
                                                @php
                                                $user = App\Models\User::find($getProduct->user_id)
                                                @endphp
                                                @if(isset($user))
                                                <a target="_blank"
                                                    href="{{ route('front.product', $getProduct->slug) }}">{{ $getProduct->name}}</a>
                                                @else
                                                <a href="javascript:;">{{$getProduct->name}}</a>
                                                @endif
                                            ...
                                            <td>
                                                ...
                                                <p>
                                                        <strong>{{ __('Price') }} :</strong> {{$order->currency_sign}}{{ round($getProduct->price * 1 , 2) }}
                                                </p>
                                               <p>
                                                    <strong>{{ __('Qty') }} :</strong> {{$product['quantity']}} {{ $getProduct->measure }}
                                               </p>

                                                    @if(!empty($product['attributes']))

                                                    @foreach( array_combine(explode(',', $product['attributes']), explode(',', $product['values']))  as $key => $value)
                                                    <p>
                                            ...
                                            <td>{{$order->currency_sign}}{{ round($getProduct->price* 1 , 2) }}
                                            </td>
                                            @php
                                            $subtotal += round($getProduct->price * 1, 2);
                                            @endphp
                                    ...
                                    <tfoot>
                                        ...
                                        <tr>
                                            <td colspan="1"></td>
                                            <td>{{ __('Total') }}</td>
                                            <td>{{$order->currency_sign}} {{ round($order->pay_amount * 1 , 2) }}
                                            </td>
                                        </tr>
                                    </tfoot>
                                </table>
